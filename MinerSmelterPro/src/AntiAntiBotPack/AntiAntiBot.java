...
public class AntiAntiBot extends AbstractScript {
	...
	private boolean isTaskSetUp = false;
	...
	TreeCutterTask treeCutterTask;
	// Executes one time;
	public void onStart() {
		populateSettnigsList();// Populates list of setting for different actions
		log("Anti-Anti-Bot: Don't fear Anti-Anti-Bot is here!!!");
		...
		log("Anti-Anti-Bot: Deterent Timer Loop Started");
	...
	public int onLoop() {
		...
		case GUESSINGCOUNTER:
			...
			// gets needed population size for trigerNumbers from
			// calculateDeterentStartChance and adjusts trigerNumbers list according
			...
			if (trigerNumbers.contains(trigerNumber) && minAmountOfTimeBeforeTriger < mainTimer.elapsed()) {
				...
				log("Anti-Anti-Bot: Starting Deterent Action");
		...
		case COUNTEREXECUTE:
			...
			if (!isSetToolGraber) {
				log("Anti-Anti-Bot: Making toolGraber for the task");
			...
			if (isRuningAction) {
				log("Anti-Anti-Bot: Runing Selected Action");
				switch (settingsList.get(randomSelectedAction).taskName) {
				case "WoodCutting":
					if(!isTaskSetUp)
					{
						treeCutterTask = new TreeCutterTask();
						treeCutterTask.onStart();
						isTaskSetUp = true;
					}
					treeCutterTask.onLoop();
					if(treeCutterTask.taskLengthTimer.finished())
					{
			...
			if (toolGraber != null) {
				if (toolGraber.isDone && !isRuningAction) {
					log("Anti-Anti-Bot: Anti-Anti-Bot reseting");
					toolGraber = null;
	...
	private void populateSettnigsList() {
		...
		woodCutterTask.toolNeeded.add("Tinderbox");
		woodCutterTask.toolNeeded.add("Knife");
